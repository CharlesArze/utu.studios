...
// "Metaballs" preset — made with the 21st.dev Shader Builder. Verbatim shader
// source (only the palette below was changed: the builder's yellow-green
// accent swapped for the site's own pink).
const FRAGMENT_SRC = `
...
// Palette (low -> high), site's own pink swapped in for the builder's
// yellow-green accent. Values are the hex colours above converted to
// normalized (0-1) linear-light-free sRGB triples, as the shader expects.
const COLOR_LOW: [number, number, number] = [0.00784, 0.00392, 0.03922]; // #02010A
const COLOR_MID: [number, number, number] = [0.83137, 0.83137, 0.84706]; // #D4D4D8
const COLOR_HIGH: [number, number, number] = [0.98039, 0.63529, 0.79216]; // #FAA2CA
...
export default function MetaballsBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    // Fullscreen triangle (clip-space), avoids the diagonal seam of a two-tri quad.
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array([-1, -1, 3, -1, -1, 3]), gl.STATIC_DRAW);
    const positionLoc = gl.getAttribLocation(program, "a_position");
    gl.enableVertexAttribArray(positionLoc);
    gl.vertexAttribPointer(positionLoc, 2, gl.FLOAT, false, 0, 0);

    const uColor0 = gl.getUniformLocation(program, "u_colors[0]");
    const uColor1 = gl.getUniformLocation(program, "u_colors[1]");
    const uColor2 = gl.getUniformLocation(program, "u_colors[2]");
    ...
    gl.uniform3f(uColor2, ...COLOR_HIGH);
    // scale, intensity, paramA, warp. Raised from 2.04: the centering fix (see the fragment
    // shader) didn't change per-pixel scale at all — it only stopped clipping off-centre blobs out
    // of frame, so the full 5-blob cluster reads as bigger now purely because more of it is
    // visible, not because any blob actually grew. Scaling p up here (dividing field-space by more
    // per screen pixel) zooms the whole pattern out around the same, now-correct centre, without
    // touching that centring. Starting estimate to bring the visible footprint back toward the old
    // (partially-clipped) look — tune further once seen live.
    gl.uniform4f(uShape, 2.8, 0.0, 0.57, 0.0);
    // detail, contrast, brightness, saturation — contrast 45, brightness 24, saturation 49 (/100).
    gl.uniform4f(uSurface, 2.4, 1.0, -0.26, 0.98);
    // hue, vignette, blur, grain — hue 0, vignette 0, grain 7/100.
    gl.uniform4f(uFinish, 0.0, 0.0, 0.0, 0.02);
    // seed, rotation, drift, OKLab toggle — OKLab blending on.
    gl.uniform4f(uTransform, 1.0, 2.46, 0.0, 1.0);
    // cursor off: presence 0 disables every cursor branch in the shader.
    ...
    function frame(now: number) {
      ...
      if (visible) {
        ...
        // Field offset (x, y). p=0 is now the container's own centre (see the
        // fragment shader), so this stays at (0, 0) to keep the cluster centred
